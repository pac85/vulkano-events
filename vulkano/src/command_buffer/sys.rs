...
impl UnsafeCommandBufferBuilder {
    ...
    #[inline]
    pub unsafe fn wait_events(&mut self, events: &UnsafeCommandBufferBuilderWaitEvents) {
        // If barrier is empty, don't do anything.
        if events.src_stage_mask == 0 || events.dst_stage_mask == 0 {
            debug_assert!(events.src_stage_mask == 0 && events.dst_stage_mask == 0);
            debug_assert!(events.memory_barriers.is_empty());
            debug_assert!(events.buffer_barriers.is_empty());
            debug_assert!(events.image_barriers.is_empty());
            return;
        }
        ...
        debug_assert_ne!(events.src_stage_mask, 0);
        debug_assert_ne!(events.dst_stage_mask, 0);

        vk.CmdWaitEvents(
            cmd,
            events.events.len() as u32,
            events.events.as_ptr(),
            events.src_stage_mask,
            events.dst_stage_mask,
            events.memory_barriers.len() as u32,
            events.memory_barriers.as_ptr(),
            events.buffer_barriers.len() as u32,
            events.buffer_barriers.as_ptr(),
            events.image_barriers.len() as u32,
            events.image_barriers.as_ptr(),
        );
    }
...
pub struct UnsafeCommandBufferBuilderWaitEvents {
    events: SmallVec<[vk::Event; 8]>,
    src_stage_mask: vk::PipelineStageFlags,
    dst_stage_mask: vk::PipelineStageFlags,
    memory_barriers: SmallVec<[vk::MemoryBarrier; 2]>,
    buffer_barriers: SmallVec<[vk::BufferMemoryBarrier; 8]>,
...
impl UnsafeCommandBufferBuilderWaitEvents {
    ...
    /// Merges another pipeline builder into this one.
    #[inline]
    pub fn merge(&mut self, other: UnsafeCommandBufferBuilderWaitEvents) {
        self.src_stage_mask |= other.src_stage_mask;
        self.dst_stage_mask |= other.dst_stage_mask;

        self.events
            .extend(other.events.into_iter());
        self.memory_barriers
            .extend(other.memory_barriers.into_iter());
        self.buffer_barriers
            .extend(other.buffer_barriers.into_iter());
    ...
    pub unsafe fn add_wait_event(
        &mut self,
        event: Event,
    ) {
        self.events.push(event.internal_object())
    } 
